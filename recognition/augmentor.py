...
class LicensePlateImageAugmentor:
    ...
    def __euler_to_mat__(yaw, pitch, roll):
        ...
        M = np.matrix([[c, 0.0, s], [0.0, 1.0, 0.0], [-s, 0.0, c]])

        # Rotate clockwise about the X-axis
        c, s = math.cos(pitch), math.sin(pitch)
        M = np.matrix([[1.0, 0.0, 0.0], [0.0, c, -s], [0.0, s, c]]) * M

        # Rotate clockwise about the Z-axis
        c, s = math.cos(roll), math.sin(roll)
        M = np.matrix([[c, -s, 0.0], [s, c, 0.0], [0.0, 0.0, 1.0]]) * M

        return M

    def __make_affine_transform__(self, from_shape, to_shape, rotation_variation=0.5):
    ...
    def generate_plate_image(self, plate_img):
        ...
        M = self.__make_affine_transform__(from_shape=plate_img.shape, to_shape=bi.shape, rotation_variation=0.5)
